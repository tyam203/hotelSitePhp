...
// hotelテーブルよりprefectureカラムを取得(prefectureでグループ化)
$sql =  "SELECT prefecture FROM hotel";
$sql .= " GROUP BY prefecture";
$result = $db->query($sql);
...
<main>
    <h1>国内ホテル検索</h1>
    <form method="post" action="searchResult.php" class="form">
    <!-- <form method="GET" action="searchResult.php" class="form"> -->
        <label>都道府県</label>
        <select name="prefecture">
            <?php while ($data = $result->fetch_object()): ?>
                <option value="<?= $data->prefecture ?>"><?= $data->prefecture ?></option>
            <?php endwhile; ?>
        </select>
        <br>

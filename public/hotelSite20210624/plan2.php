...
<?php while ($data = $result->fetch_object()) :?>
    ...
    <!-- $totalMinPrice = $data->min * $_SESSION['roomNumber'] * $_SESSION['stayCount'];
    $totalMaxPrice = $data->max * $_SESSION['roomNumber'] * $_SESSION['stayCount'];
    $totalPrice = $data->price * $_SESSION['roomNumber'] * $_SESSION['stayCount']; 
    echo '<p>料金:' . $totalMinPrice . '円～' . $totalMaxPrice . '円(' . $_SESSION['roomNumber'] . '部屋/' . $_SESSION['stayCount'] . '泊)</p>'; -->
    <p>宿泊8日前までキャンセル無料</p>

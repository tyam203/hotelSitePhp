<!-- searchResult.phpオリジナル版の保管庫
BETWEEN処理の動き確認までの仮倉庫 -->
...
<main>
    ...
    <p>部屋数：<?php echo $roomNumber ?>部屋</p>
    ...
    <?php
        ...
        $sql .= " JOIN stock2";
        $sql .= " ON room2.id = stock2.roomId";
        $sql .= " WHERE hotel2.prefectureId ='" . $prefectureId . "'";
        
        
        // 検索条件で日付指定している場合
        if (!empty($checkInDate)) {
            // $sql .= " AND stock2.date ='" . $checkInDate . "'";
            $sql .= " AND stock2.date BETWEEN'" . $checkInDate . "'AND'" . $checkOutBefore . "'";
        } 

        // 部屋の残数の要件を満たしているかチェック
        $sql .= " AND stock2.stock >='" . $roomNumber . "'";
        ...
        if ($result->num_rows > 0) {
            while ($data = $result->fetch_object()) {
                ...
                if (!empty($checkInDate)) {
                    ...
                    echo '<a href="plan.php?hotelId=' . $data->hotel_id . '&checkInDate=' . $checkInDate . '&stayDate=' . $stayDate . '&roomNumber=' . $roomNumber . '">宿泊プラン一覧はこちら</a>';
                }
                // 日付指定をしていない場合
                else{
                    echo '<a href="plan2.php?hotelId=' . $data->hotel_id . '&stayDate=' . $stayDate . '&roomNumber=' . $roomNumber . '">宿泊プラン一覧はこちら</a>';
                }
                echo '</li>';
                echo '<br>';
            }

...
// $sql2 = "SELECT  hotel.*, room. hotel_id AS hotelId, price.stock FROM room";
$sql2 = "SELECT hotelId, hotelName, MIN(total) AS min, MAX(total) AS max FROM (";
$sql2 .= " SELECT room. hotel_id AS hotelId, hotelName, SUM(price) AS total FROM room";
// $sql2 = "SELECT DISTINCT room. hotel_id AS hotelId, hotelName FROM room";
// $sql2 = "SELECT hotel.*, room. hotel_id AS hotelId, price.stock, MIN(price.price) AS min, MAX(price.price) AS max FROM room";
$sql2 .= " INNER JOIN hotel";
$sql2 .= " ON hotel.id = room.hotel_id";
$sql2 .= " INNER JOIN price";
...
if (!empty($checkInDate)) {
    ...
    $sql2 .= " AND price.stock >='" . $roomNumber . "'";
} 

// ホテルごとにグループ化
// $sql2 .= " GROUP BY room.id";
$sql2 .= " GROUP BY hotelId, roomId";

if (!empty($checkInDate)) {
    $sql2 .= " HAVING COUNT(date) ='" . $stayCount . "'"; 
} 
$sql2 .= " ) AS account";
    ...
    <ul>
        <?php if ($result->num_rows > 0): ?>
            <?php while ($data = $result->fetch_object()): ?>
                ...
                <?php if(!empty($checkInDate)): ?>
                <p>合計料金:<?= $data->min * $roomNumber ?>円～<?= $data->max * $roomNumber ?>円</p>
                <?php endif; ?>
